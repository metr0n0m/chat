...
CREATE TABLE IF NOT EXISTS `users` (
  ...
  `show_system_messages` TINYINT(1) NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `rooms` (
  ...
  `room_category` ENUM('permanent','user','commercial') NOT NULL DEFAULT 'user',
  `owner_id`      INT UNSIGNED,
  `max_members`   TINYINT UNSIGNED DEFAULT NULL,
  `is_closed`     TINYINT(1) DEFAULT 0,
  `closed_at`     DATETIME,
  `close_reason`  ENUM('last_left','idle','admin') NULL DEFAULT NULL,
  `created_at`    DATETIME DEFAULT CURRENT_TIMESTAMP,
