...
class Router
{
    ...
    private function dispatchAdmin(): void
    {
        ...
        if ($this->method === 'POST'   && preg_match('~^/api/admin/numera/(\d+)/clear-archive$~', $this->path, $m)) RoomManager::clearNumerArchive((int) $m[1]);
        if ($this->method === 'GET'    && $this->path === '/api/admin/owner-overview')                           AdminPanel::ownerOverview();
        if ($this->method === 'GET'    && $this->path === '/api/admin/whispers/sessions')                         WhisperController::ownerSessionList($_GET);
        if ($this->method === 'GET'    && preg_match('~^/api/admin/whispers/sessions/(\d+)$~', $this->path, $m)) WhisperController::ownerSessionDetail((int) $m[1]);
