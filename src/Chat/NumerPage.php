...
function renderParticipants() {
  ...
  if (owner) {
    ownerEl.innerHTML = `<span style="color:\${esc(effectiveColor(owner.nick_color))}">\u25cf \${esc(displayName(owner))}</span>`;
  } else {
    ownerEl.textContent = '—';
  }
  ...
  members.forEach(m => {
    ...
    d.className = 'p-item';
    d.innerHTML = `<span class="p-dot"></span><span style="color:\${esc(effectiveColor(m.nick_color))}">\${esc(displayName(m))}</span>`;
    list.appendChild(d);
  });
...
function appendMsg(m) {
  ...
  el.innerHTML = `<span class="msg-time">\${esc(t)}</span><span class="msg-sep"> \u00bb </span><em><span style="color:\${esc(effectiveColor(m.nick_color))};font-weight:600">\${esc(displayName(m))}</span>: \${esc(m.content)}</em>`;
...
function renderInviteDropdown(users) {
  ...
  candidates.forEach(u => {
    ...
    d.innerHTML = `<span style="width:7px;height:7px;border-radius:50%;background:#28a745;display:inline-block;flex-shrink:0"></span><span style="color:\${esc(effectiveColor(u.nick_color))}">\${esc(displayName(u))}</span>`;
